    ...
    if not st.session_state["authentication_status"]:
        ...
        with tabs[0]:
            if not st.session_state.forecast_df.empty:
                ...
                # --- NEW: FORECAST ACCURACY EVALUATOR ---
                    ...
                    if not hist_df.empty and not fcst_df.empty and 'date' in hist_df.columns:
                        ...
                        if not eval_df.empty:
                            ...
                            display_eval_df.rename(columns={
                                ...
                                'sales_variance': 'Variance (Sales)', 'forecast_customers': 'Predicted Cust.',
                                'customers': 'Actual Cust.', 'cust_variance': 'Variance (Cust.)'
                            }, inplace=True)

                            st.dataframe(
                                display_eval_df.set_index('Date').style.format({
                                    'Predicted Sales': '₱{:,.2f}', 'Actual Sales': '₱{:,.2f}',
                                    'Variance (Sales)': '₱{:,.2f}', 'Predicted Cust.': '{:,.0f}',
                                    'Actual Cust.': '{:,.0f}', 'Variance (Cust.)': '{:,.0f}'
                                }).bar(subset=["Variance (Sales)", "Variance (Cust.)"], align='mid', color=['#d62728', '#2ca02c']),
