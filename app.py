...
# --- MODIFIED: LSTM function is now adaptive to data size ---
@st.cache_resource
def train_and_forecast_lstm(historical_df, periods, target_col):
    ...
    if len(X) >= 50: # Use a more complex model for more data
        model = Sequential([
    ...
    else: # Use a simpler model for less data to prevent overfitting
        model = Sequential([
