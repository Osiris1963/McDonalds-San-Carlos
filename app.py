...
st.set_page_config(page_title="Forecaster v7.0", layout="wide")

def apply_custom_styling():
    st.markdown("""
    <style>
        .main > div { background-color: #1a1a1a; color: white; }
...
def get_forecast_logs(db_client):
    """Pulls past predictions for relearning logic."""
...
if db:
    ...
    with st.sidebar:
        ...
        if st.button("📊 Step 1: Forecast Customers", use_container_width=True):
            ...
            with st.spinner("🧠 Relearning & Optimizing..."):
        ...
        if st.button("🚀 Finalize & Save", type="primary", use_container_width=True, disabled=not ready):
            ...
            st.success("Forecast Logged!")

    tabs = st.tabs(["🔮 Dashboard", "💡 Intelligence"])
    with tabs[0]:
        if st.session_state.accuracy_bias:
            b = st.session_state.accuracy_bias
            st.info(f"🤖 Self-Learning Active: Adjusting predictions by {((b-1)*100):.1f}% based on recent performance.")
        
        if st.session_state.final_forecast_df is not None:
            st.dataframe(st.session_state.final_forecast_df.set_index('ds'), use_container_width=True)

    with tabs[1]:
        if st.session_state.customer_model:
            importance = pd.DataFrame({'Feature': st.session_state.customer_model.feature_name_, 'Importance': st.session_state.customer_model.feature_importances_}).sort_values('Importance', ascending=False).head(10)
