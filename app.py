...
def create_evaluation_data(historical_df, forecast_df):
    """Merges historical and forecast data and prepares it for weekly evaluation."""
    ...
    weekly_eval_df = eval_df.resample('W-SUN').sum()
    weekly_eval_df = weekly_eval_df.loc[(weekly_eval_df != 0).any(axis=1)]
    
    return weekly_eval_df.reset_index()

def plot_evaluation_graph(df, date_col, actual_col, forecast_col, title):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='lines+markers', 
        name='Forecast',
        line=dict(color='#c8102e', dash='dash', width=2),
        marker=dict(color='#c8102e', size=6)
    ))
...
if db:
    ...
    if not st.session_state["authentication_status"]:
        ...
        with tabs[2]:
            ...
            st.info("""
            This evaluator compares the **weekly forecast** against the **weekly actual results**. 
            It merges historical data with past forecasts to measure accuracy.
            - **Sales Accuracy:** Compares the `Predicted Sales` against `Actual Sales` (excluding add-on sales for a fair comparison).
            - **Customer Accuracy:** Compares `Predicted Customers` against `Actual Customers`.
            
            *You must have historical data that overlaps with a previously generated forecast period to see results here.*
            """)
            
            evaluation_df = create_evaluation_data(st.session_state.historical_df, st.session_state.forecast_df)
            
            if evaluation_df.empty:
                st.warning("No overlapping data found between historical records and the last generated forecast. Please enter actuals for a forecasted period or generate a new forecast.")
            else:
                eval_tab1, eval_tab2 = st.tabs(["Sales Accuracy", "Customer Accuracy"])

                with eval_tab1:
                    st.subheader("Weekly Sales: Forecast vs. Actual")
                    sales_fig = plot_evaluation_graph(
                        evaluation_df,
                        date_col='date',
                        actual_col='actual_sales',
                        forecast_col='forecast_sales',
                        title='Weekly Sales Performance'
                    )
                    st.plotly_chart(sales_fig, use_container_width=True)

                with eval_tab2:
                    st.subheader("Weekly Customers: Forecast vs. Actual")
                    cust_fig = plot_evaluation_graph(
                        evaluation_df,
                        date_col='date',
                        actual_col='actual_customers',
                        forecast_col='forecast_customers',
                        title='Weekly Customer Performance'
                    )
