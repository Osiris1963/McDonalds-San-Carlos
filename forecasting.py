# forecasting.py (Upgraded with Specialist Logic for ATV vs. Customers)
import pandas as pd
...
def run_day_specific_models(df_day_featured, target, day_of_week, periods, events_df):
    ...
    # --- Base Model Training: Prophet is always trained as it's our conservative baseline ---
    ...
    # --- MODELING STRATEGY SWITCH ---
    # If the target is ATV, we want a conservative forecast. We will use Prophet ONLY.
    if target == 'atv':
        forecast_df = prophet_model.predict(pd.DataFrame({'ds': future_dates}))
        day_forecast_df = forecast_df[['ds', 'yhat']]
        return day_forecast_df, prophet_model

    # --- For 'customers', we proceed with the full, trend-weighted stacked model ---
