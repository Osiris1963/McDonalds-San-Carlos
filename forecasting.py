...
def tune_model_hyperparameters(X, y, model_name='lgbm'):
    ...
    def objective(trial):
        ...
        for train_index, val_index in tscv.split(X):
            ...
            model.fit(X_train, y_train, eval_set=[(X_val, y_val)], eval_metric='rmse', callbacks=[lgb.early_stopping(10, verbose=False)])
...
def run_base_models(df_featured, target, periods, events_df):
    ...
    print(f"Tuning LightGBM for {target}...")
...
def train_stacked_ensemble(base_forecasts):
    ...
    # For a true stack, we'd use out-of-fold predictions.
    # As a robust alternative, we train a simple linear model on the future predictions.
    X_meta = final_df[[col for col in final_df.columns if 'yhat' in col]]
    
    # The "target" for the meta-learner is the average of the base models.
    # This helps stabilize the final prediction.
